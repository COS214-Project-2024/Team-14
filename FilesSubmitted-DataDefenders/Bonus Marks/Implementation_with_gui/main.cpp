...
private:
    char terrain;
    Building* building = nullptr;               // FOR LATER
    int x;
    int y;
    int traffic = 0;
    std::vector<MapNode*> neighbors;
public:
    MapNode(int x, int y, char c) {
        if (x < 0 || y < 0) return; 
        this->x = x;
        this->y = y;
        this->terrain = '#';
    ...
    void setTerrain(char c)
    {   
        if (c != 'S' && c != 'B' && c != '#' && c!='W') return; 
        this->terrain=c;
    }
    ...
    std::vector<MapNode*> getNeighbors()
    {
        return neighbors;
    }
...
{   
    ...
    std::vector<MapNode*> buildingSlots;
    ConcreteBuildingFactory* factory=nullptr;

    Map() 
    {   
        buildingSlotsSize=0;
        buildingListSize=0;
        grid = new MapNode**[square_size]; 
        for (int i = 0; i < square_size; i++) 
        {
            grid[i] = new MapNode*[square_size];
            for (int k = 0; k < square_size; k++) 
            {
                grid[i][k] = new MapNode(i, k, 'B'); 
            }
        }
        ...
        std::string line;
        int row = 0;
        while (std::getline(file, line) && row < square_size) 
        {
            for (int col = 0; col < line.size() && col < square_size; ++col) 
            {
                char terrainChar = line[col];
                if (terrainChar != ' ') 
                { 
                    grid[row][col]->setTerrain(terrainChar);
                    if(terrainChar=='B')
                    {
                        buildingSlots.push_back(grid[row][col]);
                        buildingSlotsSize++;
                    }
                }
            }
        ...
        // Connect neighbors for each node (up, down, left, right)
        for (int i = 0; i < square_size; i++) 
        {
            for (int j = 0; j < square_size; j++) 
            {
                if (i > 0) grid[i][j]->addNeighbor(grid[i - 1][j]);  // Up
                if (i < square_size - 1) grid[i][j]->addNeighbor(grid[i + 1][j]);  // Down
                if (j > 0) grid[i][j]->addNeighbor(grid[i][j - 1]);  // Left
                if (j < square_size - 1) grid[i][j]->addNeighbor(grid[i][j + 1]);  // Right
            }
        }
    }
    ...
    public:
    int buildingSlotsSize;
    int buildingListSize;
    int square_size = 20;
    ...
    {
        if(x<0 || x>=square_size || y<0 || y>=square_size) return NULL;
        return grid[x][y];
    }

    void setFactory(ConcreteBuildingFactory* fact)
    {
        factory=fact;
    }

    ConcreteBuildingFactory* getFactory()
    {
        return factory;
    }
    ...
    void print() 
    {
        for (int i = 0; i < square_size; ++i) 
        {
                    for (int j = 0; j < square_size; ++j) 
                    {
                        char terrain = grid[i][j]->getTerrain();
                        if (terrain == 'S') 
                        {
                            std::cout << "\033[44m"; 
                        }
    ...
    std::vector<MapNode*> getBuildingSlots()
    {
    ...
    void addBuilding(Building* b)
    {
        bool foundVacantSlot = false;
        for (int i = 0; i < buildingSlotsSize; i++)
        {
            if (buildingSlots.at(i)->getBuilding() == nullptr)
            {
                buildingSlots.at(i)->addBuilding(b);
                building_list.push_back(b); 
                buildingListSize++;
                foundVacantSlot = true;
                std::cout << "Building added to slot: " << i << "/" << buildingSlots.size() << std::endl;
                break; 
            }
        }
        ...
        if (!foundVacantSlot)
        {
            std::cout << "No vacant land available\n";
        }
    }
...
class OutputAdapter : public getOutputString 
{    
    private:
...
void onButtonClick(int buttonIndex, EndPoint* e) 
{   
    switch (buttonIndex) 
    {
        case 0:
        {   
            if(e->map->buildingListSize==0)
            {
                std::cout << "No Buildings to tax "  << std::endl;
                break;
            }
            std::cout << "Total Buildings Tax: "  << e->map->getBuildingList().at(0)->calculateTotalTax()<< std::endl;
            break;            
        }
        case 2:
        {   
            Building* b=e->factory->createBuilding(e->type,e->name,e->area,e->capacity,e->cost);
            e->map->addBuilding(b);
            break;
        }
        ...
        default:
        {
            std::cout << "Button " << buttonIndex + 1 << " clicked! Placeholder function called." << std::endl; 
            break;            
        }
...
void openInputPopup(EndPoint* end) 
{
    sf::RenderWindow inputWindow(sf::VideoMode(440, 300), "Input Parameters");

    sf::Font font;
    if (!font.loadFromFile("GUI/font/arial.ttf")) {
        return;
    }

    sf::Text instructionText("Enter type, name, area, capacity, cost:", font, 18);
    instructionText.setPosition(10, 10);
    instructionText.setFillColor(sf::Color::Black);

    sf::Text inputFields[5];
    std::string userInput[5] = {"", "", "", "", ""};
    int activeField = 0;

    for (int i = 0; i < 5; ++i) {
        inputFields[i].setFont(font);
        inputFields[i].setCharacterSize(16);
        inputFields[i].setFillColor(sf::Color::Black);
        inputFields[i].setPosition(10, 40 + (i * 40));
        inputFields[i].setString(userInput[i]);
    }
    ...
    while (inputWindow.isOpen()) 
    {
        ...
        while (inputWindow.pollEvent(event))
        {
            ...
            {
                if (activeField < 5 && event.text.unicode < 128) {
                    ...
                    inputFields[activeField].setString(userInput[activeField]);
                }
            } 
        ...
        for (int i = 0; i < 5; ++i) {
            inputWindow.draw(inputFields[i]);
        }
        inputWindow.display();
    }

    std::string type = userInput[0];
    std::string name = userInput[1];
    double area1;
    int capacity1;
    double cost1;
    ...
    try 
    {
        area1 = std::stod(userInput[2]);
        capacity1 = std::stoi(userInput[3]);
        cost1 = std::stod(userInput[4]);

        if ((type == "Commercial" || type == "Residential" || 
             type == "Industrial" || type == "Landmarks") && 
            area1 >= 0 && capacity1 >= 0 && cost1 >= 0) {
        } else {
    ...
    {
        std::cout << "Invalid Input: Non-numeric value entered for area,\n capacity, or cost." << std::endl;
        return;
    } 
    catch (const std::out_of_range& e) 
    {
        std::cout << "Invalid Input: Value out of range for area, \ncapacity, or cost." << std::endl;
        return;
    }
    end->area=area1;
    end->capacity=capacity1;
    end->cost=cost1;
    end->name=name;
    end->type=type;
}
...
{   
    ...
    ConcreteBuildingFactory* factory1=new ConcreteBuildingFactory();
    EndPoint* end=new EndPoint();
    end->factory=factory1;
    end->map=myMap;
    getOutputString* outputadapter= new OutputAdapter();

    const int gridWidth = 20;
    const int gridHeight = 20;
    const int tileSize = 32;  
    const int panelWidth = 400;
    const int terminalHeight = 160; 
    Building* b=end->factory->createBuilding("Commercial","name",2,2,2);
    b->calculateTotalTax();
    sf::RenderWindow window(sf::VideoMode(gridWidth * tileSize + panelWidth, gridHeight * tileSize), "City View");

    sf::Texture streetTexture, buildingTexture, inaccessibleTexture, inaccessibleTexture2, waterTexture,buildingTexture2;
    if (!streetTexture.loadFromFile("GUI/images/street.jpg") ||
        !buildingTexture.loadFromFile("GUI/images/sand_1.png") ||
        !inaccessibleTexture.loadFromFile("GUI/images/grass_1.png") ||   
        !inaccessibleTexture2.loadFromFile("GUI/images/grass_3.png") ||     
        !waterTexture.loadFromFile("GUI/images/water.jpg")||     
        !buildingTexture2.loadFromFile("GUI/images/building.png")) {
        return ;  
    }

    sf::Sprite streetSprite(streetTexture);
    sf::Sprite buildingSprite(buildingTexture);
    sf::Sprite inaccessibleSprite(inaccessibleTexture);
    sf::Sprite inaccessibleSprite2(inaccessibleTexture2);
    sf::Sprite waterSprite(waterTexture);
    sf::Sprite buildingSprite2(buildingTexture2);

    sf::RectangleShape panelBackground(sf::Vector2f(panelWidth, gridHeight * tileSize));
    panelBackground.setFillColor(sf::Color(200, 200, 200));
    panelBackground.setPosition(gridWidth * tileSize, 0);

    sf::RectangleShape terminalPane(sf::Vector2f(panelWidth - 20, terminalHeight - 20));
    terminalPane.setFillColor(sf::Color::Black);
    terminalPane.setPosition(gridWidth * tileSize + 10, gridHeight * tileSize - terminalHeight + 10); 

    std::vector<std::vector<sf::RectangleShape>> gridCells(gridHeight, std::vector<sf::RectangleShape>(gridWidth));
    for (int y = 0; y < gridHeight; ++y) {
        for (int x = 0; x < gridWidth; ++x) {
            gridCells[y][x].setSize(sf::Vector2f(tileSize, tileSize));
            gridCells[y][x].setPosition(x * tileSize, y * tileSize);
            gridCells[y][x].setFillColor(sf::Color::Transparent);
        }
    }
    ...
    sf::Text terminalText("", font, 18);
    terminalText.setFillColor(sf::Color::White);
    terminalText.setPosition(gridWidth * tileSize + 20, gridHeight * tileSize - terminalHeight + 20); 

    sf::Text headerText("Available Districs:", font, 28);  
    headerText.setFillColor(sf::Color::Black);
    headerText.setPosition(gridWidth * tileSize + 20, 20);  
    headerText.setStyle(sf::Text::Bold); 

    sf::Text district1Text("-Uptown District", font, 22);  
    district1Text.setFillColor(sf::Color::Black);
    district1Text.setPosition(gridWidth * tileSize + 20, 60);  

    sf::Text district2Text("-Downtown District", font, 22);  
    district2Text.setFillColor(sf::Color::Black);
    district2Text.setPosition(gridWidth * tileSize + 20, 90);  


    sf::Text InteractionsText("Interactions: ", font, 20);
    InteractionsText.setFillColor(sf::Color::Black);
    InteractionsText.setPosition(gridWidth * tileSize + 20, 260); 


    std::vector<sf::RectangleShape> buttons(12);
    std::vector<sf::Text> buttonLabels(12);

    for (int i = 0; i < 3; ++i) 
    {
        buttons[i].setSize(sf::Vector2f(160, 40)); 
        buttons[i].setFillColor(sf::Color(0, 154, 250)); 
        buttons[i].setOutlineThickness(2); 
        buttons[i].setOutlineColor(sf::Color(50, 50, 50)); 

        float verticalSpacing = 50; 
        buttons[i].setPosition(
            gridWidth * tileSize + 20,   
            300 + (i * verticalSpacing)  
        );

        buttonLabels[i].setFont(font);
        buttonLabels[i].setString("Button " + std::to_string(i + 1)); 
        buttonLabels[i].setCharacterSize(14);                      
        buttonLabels[i].setFillColor(sf::Color::White);
        buttonLabels[i].setPosition(
            buttons[i].getPosition().x + 10, 
            buttons[i].getPosition().y + 5
        ); 
    }

    buttonLabels[0].setString("Charge Building Taxes");
    buttonLabels[0].setCharacterSize(14);
    buttonLabels[1].setString("Customize\nBuilding");
    buttonLabels[1].setCharacterSize(14);
    buttonLabels[2].setString("Build\nBuilding");
    buttonLabels[2].setCharacterSize(14);
    ...
    while (window.isOpen()) 
    {
        ...
        while (window.pollEvent(event)) 
        {
            ...
            else if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) 
            {
                ...
                for (int y = 0; y < gridHeight; ++y) 
                {
                    for (int x = 0; x < gridWidth; ++x) 
                    {
                        if (gridCells[y][x].getGlobalBounds().contains(mousePos.x, mousePos.y)) 
                        {
                            MapNode* node = myMap->getNode(y, x);
                            if (node && node->getBuilding() != nullptr) 
                            {   
                                std::string district="";
                                if(node->getX()<=6) district="Uptown District";
                                if(node->getX()>=7) district="Downtown District"; 
                                Building* building = node->getBuilding();
                                std::stringstream ss;
                                ss << "Building Information at (" << x << ", " << y << "):\n";
                                ss << "Type: " << building->getBuildingType() << "\n";
                                ss << "Name: " << building->getName() << "\n";
                                ss << "Area: " << building->getFloorArea() << "\n";
                                ss << "Cost: " << building->getCost() << "\n";
                                ss << "District: " << district << "\n";
                                terminalText.setString(ss.str());
                            }
                            else 
                            {
                                terminalText.setString("No building at this location.");
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < 3; ++i) 
                {
                    ...
                    {   
                        ...
                        terminalText.setString(outputadapter->getCapturedOutput());
                        outputadapter->clear();
                    }
                }
            }
        ...
        for (int y = 0; y < gridHeight; ++y) 
        {
            for (int x = 0; x < gridWidth; ++x) 
            {
                ...
                if (myMap->getNode(y,x)->getTerrain() == 'S') 
                {
                    sprite = &streetSprite;
                } 
                else if (myMap->getNode(y,x)->getTerrain() == 'B') 
                {   
                    if(myMap->getNode(y,x)->getBuilding()==nullptr)
                    {
                ...
                else if (myMap->getNode(y,x)->getTerrain() == '#') 
                {   
                    if(x%2==0)
                    {
                        sprite = &inaccessibleSprite;
                    }
                    else
                    {
                        sprite=&inaccessibleSprite2;
                    }
                }
                else if (myMap->getNode(y,x)->getTerrain() == 'W') 
                {
                    sprite = &waterSprite;
                }
                ...
                {
                    sprite->setScale(
                        static_cast<float>(tileSize) / sprite->getTexture()->getSize().x,
                        static_cast<float>(tileSize) / sprite->getTexture()->getSize().y
                    );
                    sprite->setPosition(x * tileSize, y * tileSize);
                    window.draw(*sprite);
                }
                
                window.draw(gridCells[y][x]);
            }
        }

        window.draw(panelBackground);
        window.draw(headerText);
        window.draw(InteractionsText);
        window.draw(district1Text);
        window.draw(district2Text);
        for (int i = 0; i < 3; ++i) 
        {
            window.draw(buttons[i]);
            window.draw(buttonLabels[i]);
        }
        window.draw(terminalPane);
